...
### Added
- **`scripts/intellij-live-smoke.ps1`** — a non-Rider live smoke harness mirroring `rider-live-smoke.ps1`, targeting IntelliJ IDEA against a Kotlin/Java codebase (defaults to the plugin repo itself). It runs the universal + Kotlin/Java navigation matrix (`ide_find_class`/`find_file`/`find_definition`/`find_references`/`type_hierarchy`/`find_implementations`/`find_super_methods`/`call_hierarchy`/`search_text`/`diagnostics`/`sync_files`) and, under `-TestRename`/`-TestSafeDelete`, exercises the mutating tools against dedicated throwaway scratch files in an indexed test source root (created, verified on disk, and deleted by the script). It never mutates tracked sources and never runs `git reset --hard`. Symbol positions are computed dynamically from source text so the matrix tolerates line drift. The rename mutation lane live-verifies the PR #170 regressions on a non-Rider IDE: cursor-on-usage renames the referenced declaration (Reg 1) and `line:0,column:0` is rejected as an invalid position (Reg 2).

### Fixed
- **`ide_refactor_rename` no longer renames the enclosing declaration when the cursor is on a usage.** `inspectNamedElementCandidateForRename` selected the first enclosing declaration (`localCandidate ?: referenceCandidate`) even when the cursor sat on a reference, so renaming at `target.process()` could rename the containing method instead of `process`. Selection is now prioritized for position-based requests: (1) a declaration whose own name identifier is under the cursor, (2) a reference resolved under the cursor, (3) an enclosing-declaration fallback. Offset-less callers keep the historical `localCandidate ?: referenceCandidate` ordering. This restores correct reference-rename behavior for every language.
- **`ide_refactor_rename` no longer turns `line:0, column:0` into a silent file rename.** `resolveRenameMode` normalized non-positive coordinates to "absent", so an explicit `line:0, column:0` was treated as a (destructive) file rename. It now distinguishes truly omitted/blank coordinates (→ file rename) from explicit non-positive/non-numeric values (→ `Invalid position` error), failing closed instead of mutating a file.
- **Navigation tools again reject mutually exclusive target groups.** When a request supplied both a complete `file`+`line`+`column` *and* a complete `language`+`symbol`, position silently won. `resolveLookupMode` now returns `AMBIGUOUS` for that combination and `resolveElementFromArguments` rejects it with `Cannot specify both language+symbol and file+line+column.` Schema-default fields (`file:"", line:0, column:0`) still do not count as a position target, so symbol-only requests with default position fields are unaffected. The Rider C# backend's own position-vs-symbol preference is intentionally left unchanged. The previous implementation hardcoded `affectedFiles` to the single file observed by the post-rename mutation probe, collapsing 5+ file mutations (AXAML/code-behind pairs, partial classes, callers) to a count of 1 (Reg 4 in the May 2026 smoke). `RenameSymbolTool` now snapshots project file last-modified timestamps before the dialog automation runs and diffs against a post-rename snapshot, populating `affectedFiles` and `changesCount` from the disk delta. Falls back to the single-file probe observation only when the diff is empty. The walker prunes hidden directories and standard build output (`bin`, `obj`, `out`, `target`, `node_modules`, `build`, `dist`, `.gradle`).
...
- **`ide_refactor_rename` no longer silently falls through to the generic rename lane when Rider C# planning fails.** `RenameSymbolTool.planProductionRiderRename` now fails closed: `ProcessCanceledException`/`CancellationException` propagate, and any other planning failure returns a `BLOCKED` plan (preserving the editor lookup for cleanup) instead of throwing into a `catch` that ran the language-agnostic `RenameProcessor`. The `doExecute` planning catch and Phase-2 execution catch rethrow cancellation and return `unsupported_context` on unexpected failures rather than running the generic processor. Auto-opened declaration editors are now closed on the blocked/fail-closed path (previously leaked), and `lookupRiderFrontendEditor` closes a file it auto-opened if a later step throws.
- **`ide_refactor_rename` Rider mutation oracle now verifies the requested rename, not "something changed".** `verifyRiderFrontendMutation` previously reported success on any name change or any file-text change; it now requires the resolved target's name to equal the requested `newName` (failing closed when a concrete-but-different name resolves), keeps the path-evidence fallback for invalidated pointers (type renames that also rename the file), and only accepts a strict textual fallback (new identifier present, old identifier gone) when no name can be resolved. The disk-timestamp diff that populates `affectedFiles` now takes its post-rename snapshot **after** `commitDocuments`+`saveAllDocuments` so multi-file edits are no longer missed because on-disk mtimes hadn't been flushed.
- **Rider C# member matching is now case-sensitive.** `IndexMcpBackendHost` member-name comparison and constructor detection used `OrdinalIgnoreCase`, which could resolve the wrong overload/member in case-sensitive C#. These now use `Ordinal`.
- **`ide_find_references` (Rider) now applies the requested `scope` before truncating to the result limit.** The backend previously took the first N results and *then* filtered by path scope, so an in-scope reference past the cap was dropped. `CollectScopedReferenceInfos` now filters inside the finder consumer and counts only in-scope distinct references toward a references-specific over-collect limit (500, capped at 5000), and `totalCount` reflects the post-scope count. The Kotlin handler over-collects and paginates through `PaginationService`.
- **`ide_type_hierarchy` and `ide_find_implementations` (Rider) now honor the requested `scope`.** The already-received `scope` is applied to the inheritor (subtypes) search and implementation results before truncation. Supertypes are intentionally **not** scope-filtered: they are the type's definitional inheritance chain (commonly library types such as Avalonia's `Window` or `System.Object`), so scope-filtering them would erase the real base classes — they are always returned.
- **`ide_refactor_safe_delete` (Rider) now detects blocking usages semantically instead of via a raw regex text scan.** Usage discovery moved into `IndexMcpBackendHost.CollectSafeDeleteExternalUsages`, which resolves the declared element and runs a ReSharper reference search under a read lock before the write-lock deletion; `SafeDeleteMutationExecutor` now consumes the pre-computed usages (regex `FindBlockedUsages`/`FindProjectRoot` removed). Semantic discovery requires a live solution and is exercised by the live smoke harness; the executor unit tests validate the blocking/allow plumbing with supplied usages.
- **`ide_refactor_safe_delete` (Rider) now fails closed when the target cannot be semantically resolved.** `CollectSafeDeleteExternalUsages` previously returned an empty usage list both when a target genuinely had no external usages *and* when `ResolveDeclaredElementAt` returned null (resolution failure), so `HandleSafeDelete` could delete a still-referenced file without ever verifying usages. It now returns null to signal "unresolved", and `HandleSafeDelete` refuses the deletion (`unsupported_context`) unless `force=true`.
- **`ide_find_definition` now surfaces the Rider backend's specific `errorMessage`** instead of a generic "definition not found" message, mirroring the references path.
- **Version constants reconciled to `4.20.1`.** `McpConstants.SERVER_VERSION_FALLBACK` (was `4.20.0`) and the Rider backend `BackendVersion` (was `4.18.0`) now align with `gradle.properties:pluginVersion`.

### Changed
- **The .NET backend DLL is now staged into the real sandbox plugin directory.** `build.gradle.kts` copied `dotnet/` into `<sandbox>/plugins/<pluginName>/` using the human-readable plugin *name* ("IDE Index MCP Server") instead of the actual sandbox plugin directory (`jetbrains-index-mcp-plugin`), so `runIde` never loaded the freshly built backend. It now uses `rootProject.name`. The redundant `buildPlugin` `from(dotNetOutputDir).into("dotnet")` copy was removed: `buildPlugin` zips the prepared sandbox, which already contains the DLL at `<rootProject.name>/dotnet`, so the extra copy produced a duplicate-ZIP-entry build failure.
- **`ide_refactor_rename` server description and `SERVER_DESCRIPTION` now mention Rider and C#.** The advertised capability text omitted the Rider/.NET backend.
- **`scripts/rider-live-smoke.ps1` no longer runs `git reset --hard` implicitly.** `Reset-WorkspaceClean` now fails closed (prints the dirty file list and exits non-zero) unless the new `-ResetWorkspace` switch is passed, preventing accidental loss of uncommitted work.
...
- **Rider backend dedup (no behaviour change).** Four audit items consolidated to remove ~120 lines of duplication: (1) `ResolveSymbolIndexedForFindReferences` now delegates to `ResolveSymbolIndexed` and only wraps the outcome — the find-references path used to re-implement the entire container + member + dotted-split resolution flow byte-for-byte; the dead `ContainerCandidateResolution` record and the `ResolveContainerCandidatesCoreDetailed` indirection were removed alongside. (2) The three `rdSymbolToXData` converters in `RiderDotNetHandlers.kt` plus the inline super-methods builder now share a `rdSymbolToBaseData(symbol)` helper instead of repeating seven `RdProtocolBridge.getProperty` reads each. (3) The single-use 4-param `FormatFindReferencesUnresolvedTargetMessage(IndexedSymbolResolution)` wrapper was inlined and removed. (4) The duplicated `.ContinueWith` write-lock fault-to-Blocked pattern in the mutation handlers was extracted into `ExecuteWriteLockedMutation<TResult>` (now used by the surviving Move/SafeDelete handlers, after the rename handlers were removed — see Removed below).

### Removed
- **Dead Rider C# "backend rename" lane removed (no behaviour change).** Since the C# rename pivot to frontend dialog automation, the rd-protocol backend-rename subsystem was unreachable (`shouldUseRiderBackendRename()` was hard-wired to `false`) yet still carried across the Kotlin handler, the .NET backend, the rd protocol model, and ~250 source-introspection test assertions that merely pinned the dead code in place. The entire lane has been deleted:
 - **Kotlin `RenameSymbolTool.kt`:** removed `shouldUseRiderBackendRename`, `shouldFallbackToFrontendForCompletedRiderSymbolRename`, the `RiderRenameOutcome` sealed interface, `tryExecuteRiderSymbolRename`/`tryExecuteRiderFileRename`/`mapRiderRenameResult`, `verifyRiderFrontendExactTargetConsistency`, the `FrontendExactTargetCheck` data class, the `RIDER_BACKEND_EDITOR_SYNC_DELAY_MS` constant, the now-orphaned private `toMutationVerification`/`refreshAffectedFiles` wrappers (whose only callers were the removed mappers), and the now-unused `backendStatus`/`backendDiagnostics`/`verification` parameters threaded through `summarizeFrontendRenameResult` and `buildBlockedRiderFrontendFallbackResult`. The live frontend-automation rename lane (including the `STATUS_UNSUPPORTED` signal and the fail-closed mutation oracle) is unchanged.
 - **Kotlin `RiderDotNetHandlers.kt`:** removed the dead `RiderBackendMutationResultMapper`, `RiderBackendRenameDiagnostics`, `RiderBackendMutationResult`, and `RiderBackendVerificationResult` types and the dead `renameSymbol` rd-call timeout entry.
 - **.NET backend (`IndexMcpBackendHost.cs`):** removed the `RenameSymbol`/`RenameFile` rd registrations and their handlers/executors (`HandleRenameSymbol`, `ExecuteRenameSymbol`, `HandleRenameFile`, `ExecuteRenameFile`), the blocked-result mappers, `TryAcquireTextControlBeforeWriteLock`, `ResolveExactDeclarationNodeAt`, the rename workflow/data-context/runtime-binding helpers, the rename-only helper types (`RenameExecutionAttempt`, `RuntimeFileRenameProviderBinding`, `RefactoringDriverWithConflicts`), the `RenameRefactoringService` constructor injection, the orphaned `GetProjectFileForPath` helper and `TextControlPollTimeoutMs`/`TextControlPollIntervalMs` constants (only consumed by the removed text-control polling), and the `RuntimeRenameTargetBinding` source + test. Shared helpers used by the live Move/SafeDelete paths (`ExecuteWriteLockedMutation`, `MatchesScope`) were preserved.
 - **.NET `RenameMutationPlanner.cs` / `ExactTargetResolver.cs`:** removed `RenameMutationPlan`, `PlanExactSymbolRename`, `PlanExactFileRename`, and the `ResolveExactTarget`/`ResolveRenameTarget` wrappers (`ResolveContract` now calls `ResolveExactTargetCore` directly). The duplicated `TypeDeclarationPattern` regex was consolidated to a single `internal` field on `ExactTargetResolver`. `ReadDeclaredTypeNames` (used by SafeDelete) and the other resolver/file-target helpers used by Move/SafeDelete are retained.
 - **rd protocol model:** removed the `renameSymbol`/`renameFile` calls and the `RdRenameSymbolRequest`/`RdRenameSymbolResult`/`RdRenameFileRequest`/`RdRenameFileResult` structs and regenerated the stubs. `RdSourcePosition` (used by find-definition/find-references) and `RdMutationVerification` (used by move/safe-delete results) are retained.
 - **Tests:** pruned the dead-code-pinning source-introspection tests across the Kotlin and .NET suites while keeping the behavioral coverage (routing, mutation-oracle, result-mapper, and resolver-contract tests). Kotlin unit suite green; .NET suite 62/62.
...
- **`serverInfo.version` now matches the loaded plugin descriptor.** `McpConstants.SERVER_VERSION` was a hardcoded `"4.18.0"` string that drifted from `gradle.properties:pluginVersion` across releases (the May 2026 smoke surfaced this when v4.20.0 zips reported "MCP version 4.18.0"). `getServerVersion()` now reads the live `PluginManagerCore` descriptor at runtime and only falls back to a constant if the descriptor cannot be resolved. The legacy `SERVER_VERSION` constant is deprecated and points at the fallback so the version can never silently lag again.
- **Rider 2025.3 compile classpath: `lib/product.jar` is now extracted alongside `lib/product-client.jar`.** The upstream `2025.3` platform bump moved `com.jetbrains.rd.protocol.SolutionExtListener`, `Solution`, and `getOrCreateExtension` from `lib/product-client.jar` (no longer present in the distribution) into `lib/product.jar`, breaking `compileKotlin` on the Rider source set after `./gradlew clean buildPlugin`. The `extractRiderModelJar` Copy task and the local-Rider lookup now include both jar names; only one exists per Rider version and the missing entry is a no-op.
...
- **`scripts/rider-live-smoke.ps1` now asserts the reported MCP version, exercises four previously-unsmoked universal tools, and pins four open Rider regressions.** The script reads `pluginVersion` from `gradle.properties` (or accepts `-ExpectedVersion`) and fails if the running server reports a different version — catching the kind of drift that hid the v4.18.0/v4.20.0 mismatch in the May 2026 smoke. Default-flow now covers `ide_sync_files`, `ide_find_file`, `ide_search_text` (including a paginated page-2 fetch via cursor with a `stale:false` assertion), and `ide_diagnostics`; `-FullMatrix` additionally covers `ide_find_file` wildcard and `ide_call_hierarchy` symbol form. A new "Known regression" block pins the current broken-state symptoms for (a) bare-class `ide_find_definition` symbol form, (b) dotted `Class.Property` `ide_find_definition` symbol form, (c) `ide_find_references` symbol-form raw-`RdFault` backend-path leak, and (d) `ide_refactor_rename`'s `affectedFiles: [1]` underreport vs ≥5 files actually touched on disk. Any future fix to any of those flips the smoke red and forces an update of the assertion — converting silently-dodged failures into tracked failures.
