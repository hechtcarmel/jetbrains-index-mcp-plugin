...
import com.github.hechtcarmel.jetbrainsindexmcpplugin.tools.project.SetLifecycleLogFileTool
...
class ProjectWindowToolsUnitTest : TestCase() {
    ...
    fun testSetLifecycleLogFileToolEnabledIsRequired() {
        val required = SetLifecycleLogFileTool().inputSchema["required"]
            ?.jsonArray?.map { it.jsonPrimitive.content } ?: emptyList()
        assertTrue("enabled must be required", required.contains("enabled"))
    }
    ...
    fun testAllToolsInToolNamesAll() {
        ...
        assertTrue(ToolNames.ALL.contains(ToolNames.LIFECYCLE_LOG_FILE))
    }
    ...
    fun testProjectWindowToolsAreDisabledByDefault() {
        ...
        assertTrue("ide_set_lifecycle_log_file must be opt-in", defaults.contains(ToolNames.LIFECYCLE_LOG_FILE))
    }
}
