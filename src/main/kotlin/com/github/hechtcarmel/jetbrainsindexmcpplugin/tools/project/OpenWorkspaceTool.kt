...
class OpenWorkspaceTool : AbstractMcpTool() {
    ...
    override val description = """
        ...
        generates a temporary Maven aggregator project with symlinks, and opens it in
        IntelliJ. The result is a single project window where ide_find_references,
        ide_refactor_rename, and other tools work across all discovered modules.
        ...
        for repeated calls with the same root path.
    ...
    override suspend fun doExecute(project: Project, arguments: JsonObject): ToolCallResult {
        ...
        val outcome = withTimeoutOrNull(timeoutSeconds * 1000L) {
            ...
            val pomVf = LocalFileSystem.getInstance()
                .refreshAndFindFileByPath(File(workspaceDir, "pom.xml").absolutePath)
            if (pomVf != null) {
                linkMavenPom(opened, pomVf)
            }
        ...
        return when (outcome) {
            OpenOutcome.READY ->
                createSuccessResult(
                    "Workspace '${openedProject!!.name}' open and ready with ${mavenProjects.size} modules:\n" +
                        mavenProjects.joinToString("\n") { "  - ${it.name}" }
                )
            ...
            null -> {
                ...
                if (opened != null && !opened.isDisposed) {
                    createSuccessResult(
                        "Workspace '${opened.name}' open with ${mavenProjects.size} modules but still indexing " +
    ...
    private fun linkMavenPom(project: Project, pomVf: VirtualFile) {
        try {
            ...
            val addMethod = managerClass.getMethod("addManagedFiles", java.util.List::class.java)
            addMethod.invoke(manager, listOf(pomVf))
    ...
    private fun createWorkspace(rootPath: String, mavenProjects: List<File>): File {
        ...
        for (mvnProject in mavenProjects) {
            val link = workspaceDir.toPath().resolve(mvnProject.name)
            if (!Files.exists(link)) {
                Files.createSymbolicLink(link, mvnProject.toPath())
            }
        }

        val pomContent = generateAggregatorPom(rootPath, mavenProjects)
        File(workspaceDir, "pom.xml").writeText(pomContent)

        return workspaceDir
    }

    private fun generateAggregatorPom(rootPath: String, modules: List<File>): String {
        val dirName = File(rootPath).name
        val moduleEntries = modules.joinToString("\n") { "    <module>${it.name}</module>" }
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
  ...
  <artifactId>$dirName-workspace</artifactId>
  <version>1.0</version>
  <packaging>pom</packaging>
  <name>$dirName workspace</name>
  <modules>
$moduleEntries
  </modules>
</project>
