...
- `ide_refactor_rename` - Rename a symbol or file across the project with automatic related element renaming (getters/setters, overriding methods). Fully headless, works for ALL languages. Two modes: **symbol rename** (file + line + column or language + symbol + newName) and **file rename** (file + newName, omit line/column). Use this for name changes only, not for package relocation.
- `ide_move_file` - Move a file or directory path using the IDE's Move refactoring engine. Automatically updates all references, imports, and package declarations across the project. Use this for path-based moves across all languages.
- `ide_move_class` - Java-only class/package move tool. Accepts a target package (`targetPackage`) plus either `file` + `line` + `column` or `language` + `symbol`, and uses IntelliJ's class move refactoring instead of a raw file move.
...
- `ide_move_class` - Move a Java class to another package by package FQN (requires Java plugin)
...
4. `ToolRegistry.registerJavaRefactoringTools()` - Registers Java-only refactorings including `ide_move_class` and `ide_refactor_safe_delete` if Java plugin available
