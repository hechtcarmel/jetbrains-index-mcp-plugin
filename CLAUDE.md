...
- `ide_open_project` - Open a project by absolute path and wait until indexing completes (`timeoutSeconds`, default 600). Idempotent for already-open projects. Pass `autoLink: true` to automatically link an unlinked Maven/Gradle build system after opening. For directories without a recognized build system (TypeScript, plain directories), automatically registers the project directory as a content root so files are indexed. (disabled by default)
