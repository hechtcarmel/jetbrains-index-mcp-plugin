...
- `ide_open_project` - Open a project by absolute path and wait until indexing completes (`timeoutSeconds`, default 600). Idempotent for already-open projects — setup features (`autoLink`, `excludeDirectories`) apply on both fresh-open and already-open paths. `autoLink` (default: true) detects and links root-level Maven/Gradle, and auto-imports nested build modules in immediate subdirectories for mixed repos. `excludeDirectories` marks non-code directories (e.g. `["wksp", ".claude", "node_modules"]`) as excluded from indexing. Registers a content root automatically when the project has no build system at root. (disabled by default)
