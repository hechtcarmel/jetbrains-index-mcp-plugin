...
| `ide_set_project_mode` | Set a project's lifecycle mode (active/background/dormant/closed) | Enabled |
| `ide_get_project_modes` | List all managed projects and their current modes | Enabled |
| `ide_set_all_project_modes` | Set all managed open projects to the same mode | Enabled |
| `ide_release_project` | Remove a project from lifecycle management | Enabled |
| `ide_lifecycle_log` | Query recent lifecycle events with trigger reasons | Enabled |
...
**Enabling file output:** The ring buffer is always active. File writes (`mcp-lifecycle.log` in the IDE log directory) require enabling IntelliJ debug logging — add `#com.github.hechtcarmel.jetbrainsindexmcpplugin.lifecycle` via Help → Diagnostic Tools → Debug Log Settings. No restart needed. The file is readable directly even when no project is open.
